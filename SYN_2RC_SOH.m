% ======================================================================
% Synthetic_DRT_SOH_Dataset_2RCgen_2RCfit.m
% ----------------------------------------------------------------------
% Purpose
%   1) Generate aging-dependent synthetic DRT parameters by degradation
%      index k in [0, 1].
%   2) Convert DRT-generated TRUE ECM values directly to a 2RC truth and simulate voltage under driving profiles.
%   3) Add Markov noise to make synthetic voltage data.
%   4) Fit each synthetic voltage with a 2RC ECM. This is therefore 2RC-generation -> 2RC-fitting.
...
for sampleIdx = 1:cfg.nSyntheticSamples
    ...
    % RC_model_2. Therefore voltage generation and subsequent fitting both
    % use the same 2RC structure.
    X_true_2RC = trueECM(:);
    ...
    for fileIdx = 1:numel(cfg.driving_files)
        ...
        for seed = seedList
            ...
            rows(rowCounter,:) = { ...
                ...
                trueECM(2) + trueECM(3), max(trueECM(4:5))};
        end
    end
...
varNames = {'sample_id','k','load_id','load_name','seed','seed_name','soh_seed','markov_seed', ...
            ...
            'trueECM_Rsum','trueECM_tau_max'};

Dataset = cell2table(rows, 'VariableNames', varNames);

save(fullfile(cfg.out_dir, 'Synthetic_2RC_SOH_Dataset.mat'), 'Dataset', 'cfg');
writetable(Dataset, fullfile(cfg.out_dir, 'Synthetic_2RC_SOH_Dataset.csv'));

fprintf('\nDone. Rows = %d\n', height(Dataset));
fprintf('Saved MAT: %s\n', fullfile(cfg.out_dir, 'Synthetic_2RC_SOH_Dataset.mat'));
fprintf('Saved CSV: %s\n', fullfile(cfg.out_dir, 'Synthetic_2RC_SOH_Dataset.csv'));
