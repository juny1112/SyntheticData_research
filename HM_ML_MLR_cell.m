%% ======================================================================
%  2RC(Tbl_Load_ECM) + labels → MLR (Cell-based split + Cell-level K-fold CV)
%
%  [WHAT'S NEW vs 기존]
...
%
%  [SAMPLE STRUCTURE]
%   - sample = (cell, load_feat)   ← feature에 쓸 load만 샘플로 펼침
%   - X = 해당 load_feat의 2RC 파라미터(5×SOC_use)
%   - y = 셀 라벨을 load_feat 개수만큼 복제
%
%  [DATA SPLIT]
...
% feature에 사용할 주행부하만 선택 (여기서만 바꾸면 됨)
LOAD_FEAT_STR = "US06";         % 예: "US06" 또는 "US06 UDDS WLTP"
                                % (feature-load 선택)

% (중요) test 셀은 ID로 직접 지정
TEST_CELL_ID_STR = "02 04";     % 예: "01 13" / "1 13" / "01,13" / "01; 13"
...
fprintf('[DATA] Total samples = %d (cells %d × feat-loads %d)\n', nS, nC, nL_feat);
...
%% ── (NEW) Cell-based split: TEST cell IDs (user 지정) -------------------
...
for tt = 1:numel(target_active)
    ...
    for i = 1:nC_tv
        fold_id_of_cell(perm(i)) = mod(i-1, K) + 1;  % 1..K
    end
...
% cell_names: RowNames (cell array) from ECM tables
% TEST_CELL_ID_STR: "01 13" etc.
...
% normalize to 2-digit if numeric
...
for i = 1:numel(ids)
    ...
    % robust-ish patterns (필요 시 추가/축소 가능)
    pats = [ ...
        "x"+id, ...
        "_"+id+"_", ...
        "-"+id+"-", ...
