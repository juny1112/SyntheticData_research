...
for f = [2 3 7]%1:numel(sim_files)
    ...
    % 1) fitting 파라미터만 꺼내고, 각 seg의 중간 SOC 계산
    P_all     = para_hats(:,1:3);                           % [R0,R1,tau1]
    midSOC    = mean([SIM_table.SOC1 SIM_table.SOC2],2);    % 전체 seg 중간 SOC
    midSOC_sel= midSOC(win_idx);                            % 선택된 seg만

    % 2) SOC 그룹별 마스크
    m90 = midSOC_sel >= 80;
    m50 = midSOC_sel >= 40 & midSOC_sel < 80;
    m20 = midSOC_sel <  40;
    ...
    for g = 1:3
        idx = groups{g};
        if any(idx)
            block = P_all(idx,:);   % 해당 SOC 구간 fitting 결과
            T{r    ,:} = mean(block,1);
            T{r+1  ,:} = min (block,[],1);
            T{r+2  ,:} = max (block,[],1);
        else
...
SOC    = [20 50 90];           % x축
pNames = {'R0','R1','tau1'};  % 1-RC 파라미터 이름

for p = 1:numel(pNames)
    figure('Name',pNames{p}+" vs SOC",'NumberTitle','off');
    hold on;
