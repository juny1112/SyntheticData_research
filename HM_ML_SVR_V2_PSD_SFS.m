%% ======================================================================
%  SVR: 2RC(부하별 Tbl_<LOAD>_ECM) + PSD features + labels
...
%   - SVR: fitrsvm
%   - Standardize 옵션 사용
%   - TUNE_MODE="fixed" 또는 "random"
%% ======================================================================
clear; clc; close all;

% Text Interpreter 에러 방지
...
% random search 설정
RAND_N_TRIALS = 300;
RAND_KERNEL_SET = {'gaussian','linear'};
...
SAVE_ALL_COMBO_TABLE = true;
SAVE_BEST_BY_SIZE_TABLE = true;
...
summary_xlsx_path = fullfile(save_path, 'SVR_PSD_subset_results_by_target.xlsx');
...
for tt = 1:numel(target_active)
    ...
    kernel_col = strings(nComb,1);
    C_col = nan(nComb,1);
    eps_col = nan(nComb,1);
    ks_col = strings(nComb,1);

    feat_idx_cell = cell(nComb,1);
    cand_best_detail_cell = cell(nComb,1);
    ...
    for cc = 1:nComb
        ...
        psd_local_sel = find(bitget(mask, 1:nPSD) == 1);
        feat_idx_use = [idx_feat_ecm, idx_feat_psd(psd_local_sel)];

        [bestScore_cc, bestCand_cc, bestDetail_cc] = tuneAndEvalSubsetSVR( ...
            X_tv_full(:, feat_idx_use), y_tv, l_tv, K_FOLD, ...
            TUNE_MODE, FIXED_KERNEL, FIXED_C, FIXED_EPSILON, FIXED_KERNELSCALE, ...
            RAND_N_TRIALS, RAND_KERNEL_SET, RAND_C_RANGE_LOG10, RAND_EPS_RANGE_LOG10, RAND_KS_RANGE_LOG10, ...
            RAND_USE_AUTO_KERNELSCALE, SVR_STANDARDIZE, TUNE_METRIC, ...
            EARLYSTOP_ENABLE, EARLYSTOP_PATIENCE, EARLYSTOP_MIN_IMPROVE);

        combo_id_col(cc) = cc;
        n_psd_col(cc) = numel(psd_local_sel);
        feat_idx_cell{cc} = feat_idx_use;
        cand_best_detail_cell{cc} = bestDetail_cc;
        ...
        kernel_col(cc) = string(bestCand_cc.Kernel);
        C_col(cc) = bestCand_cc.C;
        eps_col(cc) = bestCand_cc.Epsilon;
        ks_col(cc) = string(ks2str(bestCand_cc.KernelScale));

        if isfinite(bestScore_cc)
            cv_rmse_col(cc) = bestDetail_cc.RMSE_mean;
            cv_mae_col(cc)  = bestDetail_cc.MAE_mean;
            cv_r2_col(cc)   = bestDetail_cc.R2_mean;
        end
        ...
        try
            mdl_cc = fitrsvm(X_fit_tv, y_tv, ...
                'KernelFunction', bestCand_cc.Kernel, ...
                'Standardize', SVR_STANDARDIZE, ...
                'BoxConstraint', bestCand_cc.C, ...
                'Epsilon', bestCand_cc.Epsilon, ...
                'KernelScale', bestCand_cc.KernelScale);

            if size(X_fit_te,1) >= 2
                yhat_te_cc = predict(mdl_cc, X_fit_te);
        ...
        if SEARCH_SCORE_METRIC == "MAE"
            score_cv_cc = cv_mae_col(cc);
        else
            score_cv_cc = cv_rmse_col(cc);
        end
    ...
    bestCand_final = struct( ...
        'Kernel', char(best_row_cv.Kernel), ...
        'C', best_row_cv.C, ...
        'Epsilon', best_row_cv.Epsilon, ...
        'KernelScale', parseKernelScale(best_row_cv.KernelScale{1}) );

    bestDetail_final = cand_best_detail_cell{best_idx_cv};

    fprintf('  [BEST CV SUBSET] NumPSD=%d | PSDSubset=%s | CV_RMSE=%.6f | CV_R2=%.6f | Test_RMSE=%.6f | Kernel=%s C=%.3g eps=%.3g KS=%s\n', ...
        best_row_cv.NumPSD, best_row_cv.PSDSubset, best_row_cv.CV_RMSE, best_row_cv.CV_R2, ...
        best_row_cv.Test_RMSE, best_row_cv.Kernel{1}, best_row_cv.C, best_row_cv.Epsilon, best_row_cv.KernelScale{1});
    ...
    cvRes = localRunLoadCV_SVR_WithFixedHyperparam( ...
        X_tv, y_tv, l_tv, K_FOLD, SVR_STANDARDIZE, ...
        bestCand_final.Kernel, bestCand_final.C, bestCand_final.Epsilon, bestCand_final.KernelScale);
    ...
    mdl = fitrsvm(X_tv, y_tv, ...
        'KernelFunction', bestCand_final.Kernel, ...
        'Standardize', SVR_STANDARDIZE, ...
        'BoxConstraint', bestCand_final.C, ...
        'Epsilon', bestCand_final.Epsilon, ...
        'KernelScale', bestCand_final.KernelScale);
    ...
    %% ---- Save results --------------------------------------------------
    results.(tname).best = bestCand_final;
    results.(tname).cv = cvRes;
    results.(tname).trainval.metrics = struct('R2',tv_R2,'RMSE',tv_RMSE,'MAE',tv_MAE);
    ...
    %% ---- plot ----------------------------------------------------------
    fig_all = figure('Color','w','Name',sprintf('SVR_ALL_%s_true_vs_pred',tname));
    hold on; grid on;
    ...
    if n_te >= 2
        title(sprintf('%s | SVR CV-best PSD subset | Train/Val: R2=%.3f RMSE=%.3f | CV: R2=%.3f RMSE=%.3f | Test: R2=%.3f RMSE=%.3f', ...
            tname, tv_R2, tv_RMSE, cv_R2, cv_RMSE, test_metrics.R2, test_metrics.RMSE), 'Interpreter','none');
    else
        title(sprintf('%s | SVR CV-best PSD subset | Train/Val: R2=%.3f RMSE=%.3f | CV: R2=%.3f RMSE=%.3f | Test: (none)', ...
    ...
    exportgraphics(fig_all, fullfile(save_path, sprintf('SVR_ALL_%s_true_vs_pred.png', tname)), 'Resolution', 220);
    savefig(fig_all, fullfile(save_path, sprintf('SVR_ALL_%s_true_vs_pred.fig', tname)));
...
%% ── 저장 ---------------------------------------------------------------
save(fullfile(save_path, 'SVR_results_split_LOADCV_withPSD_EXHAUSTIVE.mat'), ...
    ...
    'SVR_STANDARDIZE', 'TUNE_MODE', 'FIXED_KERNEL', 'FIXED_C', 'FIXED_EPSILON', 'FIXED_KERNELSCALE', ...
    'RAND_N_TRIALS', 'RAND_KERNEL_SET', 'RAND_C_RANGE_LOG10', 'RAND_EPS_RANGE_LOG10', 'RAND_KS_RANGE_LOG10', ...
    'RAND_USE_AUTO_KERNELSCALE', 'EARLYSTOP_ENABLE', 'EARLYSTOP_PATIENCE', 'EARLYSTOP_MIN_IMPROVE', ...
    'TUNE_METRIC', 'SEARCH_SCORE_METRIC', 'cv_best_summary_tbl', 'test_best_summary_tbl', 'summary_xlsx_path');

disp('완료: SVR ECM 고정 + PSD 모든 조합 완전탐색 + CV/test 기준 요약 + 단일 Excel 저장 완료.');
...
function ks = parseKernelScale(s)
    if isstring(s), s = char(s); end
    if strcmpi(strtrim(s), 'auto')
        ks = 'auto';
    else
        ks = str2double(s);
    end
...
function [bestScore, bestCand, bestDetail] = tuneAndEvalSubsetSVR( ...
...
function cvRes = localRunLoadCV_SVR_WithFixedHyperparam(X_tv, y_tv, l_tv, K_FOLD, doStd, kernel, C, eps0, ks)
    ...
    for k = 1:K
        ...
        try
            mdl = fitrsvm(X_tv(is_tr,:), y_tv(is_tr), ...
                'KernelFunction', kernel, ...
                'Standardize', doStd, ...
                'BoxConstraint', C, ...
                'Epsilon', eps0, ...
            ...
            yhat_k = predict(mdl, X_tv(is_val,:));
