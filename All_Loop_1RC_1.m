...
% (A) 1-RC 참 파라미터 정의
% ------------------------------------------------------------------
X_true = [0.001; 0.001; 10];          % [R0[Ω] , R1[Ω] , tau[s]]
...
para0      = [0.0012; 0.0008; 12];        % 초기 guess
lb         = [0; 0; 0.001];
ub         = para0*10;
...
for fileIdx = 1:numel(driving_files)
    ...
    % ── 2) 원본 전압 V_est 계산 ─────────────────────────────────────
    V_est = RC_model_1(X_true,t_vec,I_vec, OCV);
    ...
    subplot(3,1,2);
        ...
        title('1-RC Model Voltage'); grid on;
    subplot(3,1,3);
    ...
    % ── 4) Markov Noise + Non_noise 세트 생성·저장 ───────────────────
    noisedata            = struct;
    noisedata.Non_noise  = V_est;           % ← 원본 전압
    for seed = 1:nSeeds
        rng(seed);
    ...
    % ── 5) MultiStart fitting  (Non_noise + nSeeds) ──────────────────
    ...
    for s = 1:nSeries
        ...
        legend({'Synthetic','True','Fitted','Initial'}, ...
            'Location','northeast');
        xlabel('Time (s)'); ylabel('Voltage (V)');
    ...
    sgtitle(sprintf('%s - 1RC Fitting for All Seeds',base_name));
