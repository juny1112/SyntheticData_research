%% ======================================================================
%  2RC(부하별 Tbl_<LOAD>_ECM) + SIM driving features + labels -> MLR (Ridge + zscore)
%  + Exhaustive search over ALL SIM feature subsets
%
%  [FEATURE STRUCTURE]
%   - sample = (cell, load)
%   - X =
%       (A) 해당 load의 2RC 파라미터(5×SOC_use)  -> 항상 포함
%       (B) 해당 (cell, load)에 대응되는 SIM 특성  -> exhaustive subset search 대상
%
%  [SEARCH]
%   - ECM feature는 항상 포함
%   - SIM feature 후보들(SIM_FEAT_USE)에 대해 공집합 포함 모든 조합 평가
%   - 각 조합마다 LOAD-level CV score 계산
%   - global best SIM subset 선택
%
...
%% ── SIM feature 설정 ---------------------------------------------------
USE_SIM_FEATURE = true;

simFeatPath = "G:\공유 드라이브\BSL_Data4\HNE_RPT_@50,70_251214_9\Driving\SIM_parsed\20degC\이름정렬\주행부하_특성\SIM_feature_summary_long_600s.mat";
...
if numel(SIM_FEAT_USE) ~= numel(SIM_FEAT_TRANSFORM)
    error("SIM_FEAT_USE와 SIM_FEAT_TRANSFORM 길이가 같아야 합니다.");
end

%% ── LOAD ↔ SIM_ID 매핑 -------------------------------------------------
% SOC 70 구간이 SIM9~SIM16 이라는 전제
LOAD_TO_SIMID = containers.Map( ...
    {'US06','UDDS','HWFET','WLTP','CITY1','CITY2','HW1','HW2'}, ...
    {'SIM9','SIM10','SIM11','SIM12','SIM13','SIM14','SIM15','SIM16'} );

%% ── zscore + Ridge 설정 ------------------------------------------------
DO_ZSCORE = true;
RIDGE_LAMBDA_MODE = "auto";     % "auto" | "fixed"
...
save_path = 'G:\공유 드라이브\BSL_Data4\HNE_RPT_@50,70_251214_9\Driving\SIM_parsed\ML_MLR_SIM';
if ~exist(save_path, 'dir'), mkdir(save_path); end

summary_xlsx_path = fullfile(save_path, 'SIM_subset_results_by_target.xlsx');
...
%% ── SIM feature table 로드 ---------------------------------------------
sim_tbl = table();
if USE_SIM_FEATURE
    fprintf(">> SIM feature matPath used: %s\n", simFeatPath);

    Ss = load(simFeatPath);
    if isfield(Ss, "all_rows")
        sim_tbl = Ss.all_rows;
    elseif isfield(Ss, "SIM_feature_table")
        sim_tbl = Ss.SIM_feature_table;
    else
        error("SIM feature mat에 all_rows 또는 SIM_feature_table 변수가 없습니다: %s", simFeatPath);
    end

    missSIM = ~ismember(SIM_FEAT_USE, string(sim_tbl.Properties.VariableNames));
    if any(missSIM)
        error("SIM feature table에 없는 feature: %s", strjoin(SIM_FEAT_USE(missSIM), ", "));
    end

    if ~ismember("CellName", string(sim_tbl.Properties.VariableNames))
        error("SIM feature table에 CellName 변수가 없습니다.");
    end
    if ~ismember("SIM_ID", string(sim_tbl.Properties.VariableNames))
        error("SIM feature table에 SIM_ID 변수가 없습니다.");
    end

    sim_tbl.CellName = string(sim_tbl.CellName);
    sim_tbl.SIM_ID   = string(sim_tbl.SIM_ID);

    % 셀 번호만 추출해서 매칭용 ID 생성
    sim_tbl.CellID = arrayfun(@extractCellID, sim_tbl.CellName);

    fprintf(">> SIM feature rows loaded: %d\n", height(sim_tbl));
    fprintf(">> SIM feature unique SIM_ID: %s\n", strjoin(unique(sim_tbl.SIM_ID, 'stable'), ", "));
end
...
% ECM 쪽도 셀 번호만 추출
cell_ids = arrayfun(@extractCellID, string(cell_names));
...
featSIM_names = {};
if USE_SIM_FEATURE
    for k = 1:numel(SIM_FEAT_USE)
        featSIM_names{end+1} = sprintf('SIM_%s_%s', SIM_FEAT_USE(k), upper(SIM_FEAT_TRANSFORM(k))); %#ok<AGROW>
    end
end

feat_names_all = [feat2RC_names, featSIM_names];
nFeat = numel(feat_names_all);

idx_feat_ecm = 1:numel(feat2RC_names);
idx_feat_sim = numel(feat2RC_names) + (1:numel(featSIM_names));
nSIMF = numel(idx_feat_sim);
...
for li = 1:nL_use
    ...
    if USE_SIM_FEATURE
        if ~isKey(LOAD_TO_SIMID, L)
            error("LOAD_TO_SIMID에 없는 load입니다: %s", L);
        end
        sim_id_now = string(LOAD_TO_SIMID(L));
    end
    ...
    for ci = 1:nC
        ...
        % SIM feature
        if USE_SIM_FEATURE
            m = sim_tbl.CellID == cell_ids(ci) & sim_tbl.SIM_ID == sim_id_now;

            sim_vec = nan(1, numel(SIM_FEAT_USE));

            if nnz(m) == 1
                rowSIM = sim_tbl(find(m,1,'first'), :);

                for k = 1:numel(SIM_FEAT_USE)
                    val = rowSIM{1, char(SIM_FEAT_USE(k))};
                    sim_vec(k) = applySIMTransform(val, SIM_FEAT_TRANSFORM(k), SIM_FEAT_USE(k), sim_id_now);
                end

            elseif nnz(m) > 1
                warning("SIM feature table에서 중복행 발견: cellID=%s, SIM=%s -> 첫 행만 사용", ...
                    cell_ids(ci), sim_id_now);
                ...
                warning("SIM feature 없음: cellID=%s, SIM=%s", cell_ids(ci), sim_id_now);
            end

            X(row, col+1 : col+numel(sim_vec)) = sim_vec;
            col = col + numel(sim_vec);
        end
    end
end
...
fprintf('[DATA] ECM features = %d, SIM candidate features = %d, total SIM subsets = %d\n', ...
    numel(idx_feat_ecm), nSIMF, max(1, 2^nSIMF));
...
for tt = 1:numel(target_active)
    ...
    if USE_SIM_FEATURE && nSIMF > 0
        subset_masks = 0:(2^nSIMF - 1);
    else
        subset_masks = 0;
    end

    nComb = numel(subset_masks);
    fprintf('  [EXHAUSTIVE] evaluating %d SIM subsets (including empty set)\n', nComb);

    combo_id_col = nan(nComb,1);
    n_sim_col = nan(nComb,1);
    ...
    for cc = 1:nComb
        ...
        sim_local_sel = find(bitget(mask, 1:nSIMF) == 1);
        feat_idx_use = [idx_feat_ecm, idx_feat_sim(sim_local_sel)];
        ...
        combo_id_col(cc) = cc;
        n_sim_col(cc) = numel(sim_local_sel);
        feat_idx_cell{cc} = feat_idx_use;

        if isempty(sim_local_sel)
            subset_str_col(cc) = "(none)";
        else
            subset_str_col(cc) = strjoin(string(featSIM_names(sim_local_sel)), ", ");
        end
    ...
    combo_tbl = table( ...
        combo_id_col, n_sim_col, subset_str_col, ...
        cv_rmse_col, cv_mae_col, cv_r2_col, ...
        test_rmse_col, test_mae_col, test_r2_col, ...
        lambda_col, ...
        'VariableNames', {'ComboID','NumSIMFeat','SIMSubset', ...
    ...
    if ~any(valid_combo)
        warning('  -> 모든 SIM 조합 평가 실패. target=%s 스킵', tname);
        continue;
    end
    ...
    best_feat_idx = feat_idx_cell{best_idx_cv};
    feat_names_use = feat_names_all(best_feat_idx);
    nFeat_use = numel(best_feat_idx);
    lambda_use = best_row_cv.Lambda;

    fprintf('  [BEST CV SUBSET] NumSIMFeat=%d | SIMSubset=%s | CV_RMSE=%.6f | CV_R2=%.6f | Test_RMSE=%.6f | lambda=%.3g\n', ...
        best_row_cv.NumSIMFeat, best_row_cv.SIMSubset, best_row_cv.CV_RMSE, best_row_cv.CV_R2, best_row_cv.Test_RMSE, lambda_use);

    fprintf('  [BEST TEST SUBSET] NumSIMFeat=%d | SIMSubset=%s | Test_RMSE=%.6f | CV_RMSE=%.6f\n', ...
        best_row_test.NumSIMFeat, best_row_test.SIMSubset, best_row_test.Test_RMSE, best_row_test.CV_RMSE);

    best_by_size_tbl = table();
    if SAVE_BEST_BY_SIZE_TABLE
        rows_tmp = [];
        for kk = 0:nSIMF
            mk = combo_tbl.NumSIMFeat == kk & isfinite(combo_tbl.CV_RMSE);
            if any(mk)
                tblk = combo_tbl(mk,:);
                [~, ib] = min(tblk.CV_RMSE);
    ...
    results.(tname).sim.use = USE_SIM_FEATURE;
    results.(tname).sim.features = SIM_FEAT_USE;
    results.(tname).sim.transforms = SIM_FEAT_TRANSFORM;
    ...
    if n_te >= 2
        title(sprintf('%s | CV-best SIM subset | Train/Val: R2=%.3f RMSE=%.3f | CV: R2=%.3f RMSE=%.3f | Test: R2=%.3f RMSE=%.3f', ...
            tname, tv_R2, tv_RMSE, cv_R2, cv_RMSE, test_metrics.R2, test_metrics.RMSE), 'Interpreter','none');
    else
        title(sprintf('%s | CV-best SIM subset | Train/Val: R2=%.3f RMSE=%.3f | CV: R2=%.3f RMSE=%.3f | Test: (none)', ...
    ...
    summary_cv_best_subset(end+1,1) = best_row_cv.SIMSubset; %#ok<AGROW>
    summary_cv_num_sim(end+1,1) = best_row_cv.NumSIMFeat; %#ok<AGROW>
    ...
    summary_te_best_subset(end+1,1) = best_row_test.SIMSubset; %#ok<AGROW>
    summary_te_num_sim(end+1,1) = best_row_test.NumSIMFeat; %#ok<AGROW>
...
cv_best_summary_tbl = table( ...
    summary_cv_target, summary_cv_num_sim, summary_cv_best_subset, ...
    summary_cv_rmse, summary_cv_mae, summary_cv_r2, ...
    summary_cv_test_rmse, summary_cv_test_mae, summary_cv_test_r2, ...
    'VariableNames', {'Target','NumSIMFeat','CV_BestSIMSubset', ...
    'CV_RMSE','CV_MAE','CV_R2', ...
    'Test_RMSE','Test_MAE','Test_R2'});

test_best_summary_tbl = table( ...
    summary_te_target, summary_te_num_sim, summary_te_best_subset, ...
    summary_te_cv_rmse, summary_te_cv_mae, summary_te_cv_r2, ...
    summary_te_rmse, summary_te_mae, summary_te_r2, ...
    'VariableNames', {'Target','NumSIMFeat','Test_BestSIMSubset', ...
    'CV_RMSE','CV_MAE','CV_R2', ...
    'Test_RMSE','Test_MAE','Test_R2'});

disp('=== CV-best SIM subset summary by target ===');
disp(cv_best_summary_tbl);

disp('=== Test-best SIM subset summary by target ===');
...
%% ── 저장 ---------------------------------------------------------------
save(fullfile(save_path, 'MLR_RIDGE_results_split_LOADCV_withSIM_EXHAUSTIVE.mat'), ...
    'results', 'X', 'feat_names_all', 'cell_names', 'cell_ids', 'SOC_use', 'pNames_2RC', ...
    'LOAD_USE_STR', 'LOAD_TEST_STR', 'LOAD_TRAINVAL_STR', 'K_FOLD', 'TEMP_list', ...
    'load_id', 'cell_id', 'load_use', 'load_trainval', 'load_test', ...
    'USE_SIM_FEATURE', 'simFeatPath', 'SIM_FEAT_USE', 'SIM_FEAT_TRANSFORM', ...
...
disp('완료: ECM 고정 + SIM feature 모든 조합 완전탐색 + CV/test 기준 요약 + 단일 Excel 저장 완료.');
...
function out = applySIMTransform(x, mode, featName, simName)
...
switch mode
    ...
    case "log10"
        if x > 0
            ...
            warning("log10 변환 불가: feature=%s, sim=%s, value=%.6g -> NaN 처리", ...
                string(featName), string(simName), x);
            out = NaN;
        end
    ...
    case "log1p"
        if x > -1
            ...
            warning("log1p 변환 불가: feature=%s, sim=%s, value=%.6g -> NaN 처리", ...
                string(featName), string(simName), x);
            out = NaN;
        end

    otherwise
        error("알 수 없는 SIM transform mode: %s (feature=%s)", string(mode), string(featName));
end
end

function cid = extractCellID(s)
s = string(s);
