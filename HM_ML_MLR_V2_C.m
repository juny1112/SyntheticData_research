...
save_path = 'G:\공유 드라이브\BSL_Data4\HNE_RPT_@50,70_251214_9\Driving\SIM_parsed\ML_MLR_C';
...
featBaseNames = {'R0','R1','R2','C1','C2'};

feat2RC_names = {};
for s = SOC_use(:).'
    for pi = 1:numel(featBaseNames)
        feat2RC_names{end+1} = sprintf('%s_%d', featBaseNames{pi}, s); %#ok<AGROW>
    end
end
...
for li = 1:nL_use
    ...
    for ci = 1:nC
        ...
        for s = SOC_use(:).'
            ...
            % 존재 여부 확인
            has_R0   = ismember(vn_R0,   vnames);
            has_R1   = ismember(vn_R1,   vnames);
            has_R2   = ismember(vn_R2,   vnames);
            has_tau1 = ismember(vn_tau1, vnames);
            has_tau2 = ismember(vn_tau2, vnames);

            if has_R0,   R0v   = TblL{ci, vn_R0};   else, R0v   = NaN; end
            if has_R1,   R1v   = TblL{ci, vn_R1};   else, R1v   = NaN; end
            if has_R2,   R2v   = TblL{ci, vn_R2};   else, R2v   = NaN; end
            if has_tau1, tau1v = TblL{ci, vn_tau1}; else, tau1v = NaN; end
            if has_tau2, tau2v = TblL{ci, vn_tau2}; else, tau2v = NaN; end
...
%% ── 저장 ---------------------------------------------------------------
save(fullfile(save_path, 'MLR_results_split_LOADCV_Cfeat.mat'), ...
    'results', 'X', 'feat2RC_names', 'cell_names', 'SOC_use', ...
    'LOAD_USE_STR', 'LOAD_TEST_STR', 'LOAD_TRAINVAL_STR', 'K_FOLD', 'TEMP_list', ...
    'load_id', 'cell_id', 'load_use', 'load_trainval', 'load_test');

disp('완료: C1/C2 feature 버전 LOAD split + train/val LOAD-level K-fold CV + test 평가 결과 저장.');
