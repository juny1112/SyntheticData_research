%% ======================================================================
%  SVR: 2RC(부하별 Tbl_<LOAD>_ECM) + (NEW) PSD features + labels
%
%  - sample = (cell, load)
%  - X =
...
%
%  (NEW) PSD feature 추가:
%   - PSD_feature.m에서 생성한 psd_stat_tbl(.mat)을 로드
%   - 선택한 PSD feature(예: E_fast_A2, f_mean_Hz 등)를 X에 추가
%   - PSD는 load 고유값이므로 (cell,load) 샘플에서 동일 load에 대해 동일 값이 들어감
%
%  PSD features 후보(PSD_feature.m 결과):
%   PSD_mean_A2Hz, PSD_std_A2Hz, PSD_int_A2,
%   E_fast_A2, E_slow_A2, E_slow_fast_ratio,
%   f_mean_Hz, f_std_Hz, fast_mean_Hz, slow_mean_Hz,
%   tau_mean_s, tau_fast_mean_s, tau_slow_mean_s
% ======================================================================
clear; clc; close all;
...
% 예: ["E_fast_A2","E_slow_A2","E_slow_fast_ratio","f_mean_Hz","f_std_Hz","fast_mean_Hz","slow_mean_Hz"]
PSD_FEAT_USE = ["E_slow_fast_ratio"];     % <-- 여기만 네가 바꾸면 됨

% (옵션) log1p 변환(스케일 완화): log(1+x)
PSD_LOG1P = false;
PSD_LOG_COLS = ["E_fast_A2","E_slow_A2","PSD_int_A2"];   % log1p 적용 후보(원하면 수정)
...
RAND_KERNEL_SET = {'gaussian','linear'};    % 필요시 {'gaussian','linear'} 등
% C, epsilon, kernel scale 샘플링 범위(로그 스케일 권장)
RAND_C_RANGE_LOG10        = [-3, 1];     % 1e-3 ~ 1e1 (원하면 확장)
RAND_EPS_RANGE_LOG10      = [-4, 0];     % 1e-4 ~ 1
RAND_KS_RANGE_LOG10       = [-3, 2];     % 1e-3 ~ 1e2
RAND_USE_AUTO_KERNELSCALE = true;        % true면 KernelScale auto도 후보로 섞음
...
for s = SOC_use(:).'
    for pi = 1:numel(pNames_2RC)
        featECM_names{end+1} = sprintf('%s_%d', pNames_2RC{pi}, s); %#ok<AGROW>
    end
end

% (2) (NEW) PSD feature names (prefix "PSD_")
featPSD_names = {};
if USE_PSD_FEATURE
    for k = 1:numel(PSD_FEAT_USE)
        featPSD_names{end+1} = sprintf('PSD_%s', PSD_FEAT_USE(k)); %#ok<AGROW>
    end
end

feat_names = [featECM_names, featPSD_names];
...
for li = 1:nL_use
    ...
    % (NEW) 해당 load의 PSD feature 벡터(1×nPSD) 준비 → 모든 cell에 동일하게 broadcast
    psd_vec = [];
    if USE_PSD_FEATURE
        Lkey = upper(string(L));
        if isfield(psd_byLoad, char(Lkey))
            ...
            for k = 1:numel(PSD_FEAT_USE)
                psd_vec(k) = rowPSD{1, char(PSD_FEAT_USE(k))};
            end

            % (옵션) log1p 변환
            if PSD_LOG1P
                for k = 1:numel(PSD_FEAT_USE)
                    if any(PSD_FEAT_USE(k) == PSD_LOG_COLS)
                        psd_vec(k) = log1p(psd_vec(k));
                    end
                end
            end
        else
    ...
    for ci = 1:nC
        ...
        % ---- (B) (NEW) PSD feature 붙이기 (load 고유값)
        if USE_PSD_FEATURE
...
fprintf('[DATA] feature names (%d): %s\n', nFeat, strjoin(string(feat_names), ', '));
...
for tt = 1:numel(target_active)
    ...
    results.(tname).USE_PSD_FEATURE = USE_PSD_FEATURE;
    results.(tname).PSD_FEAT_USE = PSD_FEAT_USE;
    results.(tname).PSD_LOG1P = PSD_LOG1P;
    results.(tname).PSD_LOG_COLS = PSD_LOG_COLS;
    ...
    if numel(yhat_te) >= 2
        title(sprintf('%s | CV(%s)=%.3f | Train/Val: R2=%.3f RMSE=%.3f | Test: R2=%.3f RMSE=%.3f', ...
            tname, TUNE_METRIC, bestScore, tv.R2, tv.RMSE, te.R2, te.RMSE));
    else
        title(sprintf('%s | CV(%s)=%.3f | Train/Val: R2=%.3f RMSE=%.3f | Test: (none)', ...
            tname, TUNE_METRIC, bestScore, tv.R2, tv.RMSE));
    end
...
%% ── 저장 --------------------------------------------------------------
save(fullfile(save_path, 'SVR_results_split_LOADCV_withPSD.mat'), ...
    ...
    'USE_PSD_FEATURE','psdMatPath','PSD_FEAT_USE','PSD_LOG1P','PSD_LOG_COLS', ...
...
disp('완료: SVR LOAD-split + LOAD-level CV + (PSD feature 추가) + (random search 옵션) + TEST 평가 저장');
