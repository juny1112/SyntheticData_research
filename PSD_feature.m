...
%% 0) 분석할 파일 목록 -----------------------------------------------------
driving_files = {
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\05_BSL_CITY1_0725.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\06_BSL_CITY2_0726.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\07_BSL_HW1_0725.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\08_BSL_HW2_0725.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\09_20190312_580894.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\10_20190312_7102956.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\11_20190314_530861.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\12_20190315_2601679.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\13_20190416_250622.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\14_20190504_7701441.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\15_20190505_380577.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\16_20190615_660356.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\17_20190916_3101971.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\18_20190920_4301446.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\19_20190921_770417.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\20_20191220_7501215.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\01_us06_0725.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\02_udds_0725.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\03_hwfet_0725.xlsx"
"G:\공유 드라이브\Battery Software Group (2025)\Protocols\2026 Driving_Loads_20\after_scaling\04_WLTP_0725.xlsx"
};
...
%% 사용자 설정 ------------------------------------------------------------
% t_max = 600이면 각 파일의 시작점 기준 600초만 사용
% t_max = []이면 각 파일 전체 사용
t_max    = 600;         % [s] 600초 기준 PSD feature 추출
...
for fileIdx = 1:nLoad
    ...
    if numel(t_vec) < 10
        warning('crop 이후 데이터가 너무 짧습니다: %s', filename);
        continue;
    end

    % 600초 기준으로 비교할 때, 실제 길이 확인
    duration_s = t_vec(end) - t_vec(1);
    if ~isempty(t_max) && duration_s < 0.95*t_max
        warning('설정한 t_max보다 짧은 데이터입니다: %s, duration=%.2f s', filename, duration_s);
    end
    ...
    %% (I) PSD-weighted mean/std in frequency, 전체 대역
    wSum = sum(P_stat);
    ...
    %% (J) log10-frequency representative frequency/tau/std, 전체 대역
    f_geo_mean = NaN;
    tau_geo_mean = NaN;
    f_geo_std_dec = NaN;
    ...
    %% (K) band별 PSD-weighted mean frequency
    fast_mean = NaN;
    slow_mean = NaN;
    tau_fast_mean = NaN;
    tau_slow_mean = NaN;
    ...
    %% (L) band별 log10-frequency representative frequency/tau
    f_fast_geo_mean = NaN;
    f_slow_geo_mean = NaN;
    tau_fast_geo_mean = NaN;
    tau_slow_geo_mean = NaN;
    ...
    all_psd_stat.(sprintf('file%d',fileIdx)) = struct( ...
        ...
        'E_fast',E_fast, ...
        'E_slow',E_slow, ...
        'ratio',ratio_slow_fast, ...
        'f_mean',f_mean, ...
        'f_std',f_std, ...
        'tau_mean',tau_mean, ...
        'fast_mean',fast_mean, ...
        'slow_mean',slow_mean, ...
        'tau_fast_mean',tau_fast_mean, ...
        'tau_slow_mean',tau_slow_mean, ...
        'f_geo_mean',f_geo_mean, ...
        'tau_geo_mean',tau_geo_mean, ...
        'f_geo_std_dec',f_geo_std_dec, ...
        'f_fast_geo_mean',f_fast_geo_mean, ...
        'tau_fast_geo_mean',tau_fast_geo_mean, ...
        'f_slow_geo_mean',f_slow_geo_mean, ...
        'tau_slow_geo_mean',tau_slow_geo_mean, ...
        'tau_fast_range',tau_fast_range, ...
        'tau_slow_range',tau_slow_range, ...
        'f_fast',f_fast, ...
        'f_slow',f_slow);
    ...
    fprintf('--- %s ---\n', load_label);
    fprintf('duration=%.3f s, fs=%.6g Hz, dt=%.6g s, nWin=%d, nfft=%d\n', ...
        duration_s, fs, dt, nWin, nfft);
    fprintf('df=%.9g Hz, fmin=%.9g Hz, tau_max≈%.1f s\n', ...
        df, fmin_pos, tau_max_effective);
    ...
    %% (Q) Figure 1개/파일
    fig_title = sprintf('[%d] %s', fileIdx, load_label);
    ...
    % time domain
    subplot(2,1,1);
    plot(t_vec, I_vec, 'LineWidth', 1.1);
    grid on;
    xlabel('Time (s)');
    ylabel('Current (A)');

    if isempty(t_max)
        title(sprintf('%s Current (full length)', load_label), 'Interpreter','none');
    else
        title(sprintf('%s Current (0~%d s)', load_label, round(t_max)), 'Interpreter','none');
    end

    % PSD
    ax = subplot(2,1,2);
    hP = plot(f(mPlot), yplot(mPlot), 'LineWidth', 1.2);
    grid on;
    hold on;
    ...
    if isgraphics(hFastPatch)
        ...
        legL(end+1) = sprintf("FAST band (tau=%g~%g s)", tau_fast_range(1), tau_fast_range(2));
    end

    if isgraphics(hSlowPatch)
        legH(end+1) = hSlowPatch; %#ok<AGROW>
        legL(end+1) = sprintf("SLOW band (tau=%g~%g s)", tau_slow_range(1), tau_slow_range(2));
    end
