...
%  - label(타겟)도 pool/active 구조로 확장 가능
%
%  - MLR 설정:
...
%% ── 타겟 이름 / 벡터 매핑 ----------------------------------------------
target_pool_names = {'QC2','QC40','Rcharg','Rcharg_80_90_avg','R1s'}; %#ok<NASGU>
...
for tt = 1:numel(target_active)
    ...
    if abs(y_mean) > eps
        ...
        RMSE_percent = NaN;
        warning('타겟 %s: mean(y_true)가 0에 가까워 %%RMSE 계산 불가', tname);
    end
    ...
    results.(tname).RMSE_percent  = RMSE_percent;
    ...
    % ---- 예측 vs 실제 scatter plot만 표시 ----
    figure('Color','w','Name',sprintf('MLR_%s_true_vs_pred',tname));
    ...
    title(sprintf('MLR: %s | Loads: %s (R^2=%.3f, RMSE=%.3f, %%RMSE=%.2f%%)', ...
        tname_tex, strjoin(load_use, ', '), R2, RMSE, RMSE_percent), ...
