...
% 결과 저장 경로 (MLR)
save_path = 'G:\공유 드라이브\BSL_Data4\HNE_agedcell_8_processed\SIM_parsed\2RC_fitting\ML_MLR';
...
for t = 1:numel(target_active)
    ...
    %% ---- 계수 테이블 CSV 저장 -----------------------------------------
    coef_tbl = mdl.Coefficients;   % RowNames: (Intercept), x1,x2,... / Name: Estimate, SE, tStat, pValue
    coef_tbl.Properties.Description = sprintf('MLR coefficients for target %s', tname);
    ...
    title(sprintf('MLR: %s (R^2=%.3f, RMSE=%.3f)', tname_tex, R2, RMSE), ...
